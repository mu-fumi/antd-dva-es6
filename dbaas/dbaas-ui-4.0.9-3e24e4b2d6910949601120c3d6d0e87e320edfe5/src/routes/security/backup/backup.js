...
class Backup extends Base{
  ...
  handleBackupReload() {
    this.props.dispatch({
      type:'backup/handleKeyword',
      payload:{
        keyword: this._keyword
      }
    })
    this.props.dispatch({
      type : 'backup/handleReload'
    })
  }
  render(){
    const { location,backup,dispatch } = this.props;
    const { placeholder, keyword, reload, pickerDefaultValue, instances, node } = backup;
    const searchProps = [{
      keyword,
      placeholder,
      onSearch(value) {
        dispatch({
          type: 'backup/handleFilter',
          payload: {
            keyword:value
          },
        })
    ...
    const rangePickerProps = [
      {
        ...
        props: {
          ...
          onChange(value, dataString) {
            dispatch({
              type: `backup/handlePublishRange`,  // 接口参数
              payload: dataString
            })
            dispatch({
              type: `backup/handlePickerDefaultValue`,  // picker的defaultValue，便于跳转别的页面然后从别的页面返回本页时保留筛选条件
              payload: value
            })
            dispatch ({
              type: `backup/setCurrentPage`,
              payload: 1
            })
          }
        }
