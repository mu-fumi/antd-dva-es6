...
class Set extends Base {
  constructor(props) {
    ...
    this.push({
      type: 'set/resetFilter',
      fire: [Base.WillUnmount]
    })

    this.push({
      type : 'set/resetDefaultBelongs',
      fire: [Base.WillUnmount]
    })
  }
  ...
  handleOffLine() {
    ...
    window.open(`/offline/types?tag=set&ids=${selectedIds.join(',')}`)
  }
  ...
  handleClusterReload() {
    ...
    this.clearQuery()
    this.props.dispatch({
      type : 'set/handleFilter',
      payload : {keyword : this._keywords}
    })
    this.props.dispatch({
      type: 'set/handleReload'
    })
  }

  handleHoverNode(id) {
    this.props.dispatch({
      type: 'set/getNodeStatus',
      payload: id
    })
  }
  ...
  render() {
    ...
    const {
      ...
      selectedIds, defaultBelongs, belongsOptions
    } = set
    ...
    let columns = [{
      ...
      render: (text, record) => {
        return <Row className={styles['name-container']}>
          <Link to={`/cmdb/instance-group/${record.id}`}>{text}</Link>
          &nbsp;
          <Link to={`/graphs?set_id=${record.id}`} target="_blank">
            <Tooltip title="查看监控">
      ...
      render: (text, record) => {
        ...
        return <Link to={`/nodes?relate_id=${record.id}&relate_type=1`}>{text}</Link>
      }
    }, {
      title: cpuTitle,
      dataIndex: 'cpu_avg',
...
export default connect((state) => {
  return {
    loading: state.loading.models['set'],
    set: state['set']
  }
})(Set)
